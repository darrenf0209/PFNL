function [average_p, average_s] = evalVideo(fpath, model, frames_foregone)
...
for j = 1:length(list0)
    ...
    if frames_foregone > 0
        path1 = strcat(kind, '/truth_downsize_2_frames_foregone/');
    else
        path1=strcat(kind,'/truth_downsize_2/');
    end
...
%         fprintf('%d %f\n',i,psnr);
        ...
        if frames_foregone > 0
            ...
            if i == length(list1) - 1
                fprintf(fid, strcat('\"%d\":[%g,%g]'),i, psnr, ssim);
            else
                fprintf(fid, strcat('\"%d\":[%g,%g],'),i, psnr, ssim);
            end
        end
